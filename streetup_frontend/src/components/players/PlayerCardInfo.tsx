...
export default function PlayerCardInfo({player_id}: PlayerCardInfoProps ) {
  
  const {data: profile} = useQuery<Profile>({
    queryKey: ["profile",player_id],
    queryFn: ()=> getProfileById(player_id),
    retry: 2
  })


  const {data: teams} = useQuery<Team[]>({
    queryKey: ["teams",player_id],
    queryFn: ()=>getTeamsByLeader(player_id),
    retry: 2,
  });
          ...
          <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:border-fuchsia-500/30 transition-all duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="font-bold text-white">...PROXIMAMENTE...</p>
              </div>
